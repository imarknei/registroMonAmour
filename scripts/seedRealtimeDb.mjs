...
async function seed() {
  console.log('🚀 Conectando a Firebase Realtime Database (bidmark-race)...');

  // 1. Guardar Tarifas
  console.log('📦 Guardando tarifas oficiales...');
  await set(ref(db, 'motel_config/tariffs'), INITIAL_TARIFFS);

  // 2. Guardar Habitaciones
  console.log('🛏️ Guardando 12 habitaciones...');
  const roomsMap = {};
  INITIAL_ROOMS.forEach(r => { roomsMap[r.id] = r; });
  await set(ref(db, 'rooms'), roomsMap);

  // 3. Guardar Productos
  console.log('🛒 Guardando inventario de 24 productos...');
  const productsMap = {};
  INITIAL_PRODUCTS.forEach(p => { productsMap[p.id] = p; });
  await set(ref(db, 'products'), productsMap);

  console.log('✅ ¡TODAS LAS HABITACIONES, PRODUCTOS Y TARIFAS FUERON SUBIDAS CON ÉXITO A FIREBASE!');
  process.exit(0);
}
