...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  const handleUserSelect = (user: typeof SYSTEM_USERS[0]) => {
    ...
    if (user.role === 'admin') {
      ...
      // Si se selecciona un recepcionista, revocar inmediatamente cualquier sesión de administrador activa
      if (isAdminAuthenticated) {
        onLockAdmin();
      }
      setCurrentUserById(user.id);
        ...
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          <div className="hidden md:flex items-center gap-4 lg:gap-6">
            ...
            {currentUser.role !== 'admin' && currentShift && (
              <div
                className="flex items-center gap-2.5 bg-rose-50/80 border border-rose-200/80 rounded-xl px-3 py-1.5 shadow-xs text-left"
              >
