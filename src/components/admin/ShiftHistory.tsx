...
export const ShiftHistory: React.FC = () => {
  ...
  const handleSyncFromCloud = async () => {
    setIsSyncing(true);
    try {
      const data = await reloadFromFirebase();
      const count = data?.shifts?.length || shiftsHistory.length;
      showToast({
        title: 'Sincronización Exitosa',
        message: `Se cargaron ${count} turnos e historial completo desde la base de datos.`,
        type: 'success',
      });
    } catch (err) {
      showToast({
        title: 'Error de Sincronización',
        message: 'No se pudo contactar a la base de datos.',
        type: 'error',
      });
    } finally {
      setIsSyncing(false);
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2 bg-slate-50 p-2 rounded-2xl border border-slate-200">
            ...
            <button
              ...
              disabled={isSyncing}
              className="px-3 py-1.5 rounded-xl text-xs font-extrabold bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white shadow-sm transition-all flex items-center gap-1.5 ml-auto disabled:opacity-50"
              title="Refrescar y recargar todo el historial de turnos directamente de la base de datos"
            >
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-xl text-xs font-extrabold bg-sky-600 hover:bg-sky-700 active:scale-95 text-white shadow-sm transition-all flex items-center gap-1.5"
          ...
          <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="pt-2">
              <button
                ...
                onClick={handleSyncFromCloud}
                disabled={isSyncing}
                className="px-4 py-2 bg-brand-600 hover:bg-brand-700 active:scale-95 text-white font-extrabold text-xs rounded-xl shadow-md transition-all inline-flex items-center gap-2"
              >
                <RefreshCw className={`w-4 h-4 ${isSyncing ? 'animate-spin' : ''}`} />
                <span>{isSyncing ? 'Recargando...' : 'Sincronizar y Cargar Historial desde la Nube'}</span>
              </button>
            </div>
          </div>
        ) : (
